...
@tree.command(name="bedwars", description="Get overall BedWars stats for a Hypixel player")
@app_commands.describe(player="Minecraft username")
async def bedwars(interaction: discord.Interaction, player: str):
    ...
    stats = parse_bedwars(player_data)

    embed = discord.Embed(
        title=f"BedWars Stats  ·  {player}",
        color=0xFFAA00,
    )
